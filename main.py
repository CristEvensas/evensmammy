...
async def start(message: types.Message):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			await message.answer('❤️Добро пожаловать', reply_markup=kb.menu)
		else:
			await message.answer('❤️Приветствую, это бот-саппорт.\nНапиши мне свой вопрос и я отправлю его администрации.\nЗа спам/флуд - ЧС!')
	else:
		await message.answer('😲Ты получил бан от администрации😲')
...
async def hanadler(message: types.Message, state: FSMContext):
	func.join(chat_id=message.chat.id)
	q.execute(f"SELECT block FROM users WHERE user_id = {message.chat.id}")
	result = q.fetchone() #СЛИТО В @EDITSOFTED
	if result[0] == 0:
		if message.chat.id == admin:
...
async def hfandler(message: types.Message, state: FSMContext):
	...
	q.execute(f"SELECT block FROM users WHERE user_id = {message.chat.id}")
	result = q.fetchone()
	if result[0] == 0: #СЛИТО В @EDITSOFTED
		if message.chat.id == admin:
...
async def hangdler(message: types.Message, state: FSMContext):
	...
	result = q.fetchone()                     #СЛИТО В @EDITSOFTED
	if result[0] == 0:
		if message.chat.id == admin:        #СЛИТО В @EDITSOFTED
	...
	if message.text == '⏪ Отмена':
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				a = result[0]
				id = a[0]                            #СЛИТО В @EDITSOFTED
				if id == 0:
					q.execute(f"UPDATE users SET block = 1 WHERE user_id = {message.text}")
					connection.commit()
					await message.answer('Пользователь успешно Ёбнулся.', reply_markup=kb.adm)
					await state.finish()
		...
		if message.text.isdigit():
			...
			if len(result) == 0:
				...
				if id == 1:
					...
					await state.finish()
					await bot.send_message(message.text, 'Вы были разблокированы администрацией. СЛИТО В @EDITSOFTED')
				else:
					await message.answer('Данный пользователь не получил бан. СЛИТО В @EDITSOFTED', reply_markup=kb.adm)
					await state.finish()
		else:
			await message.answer('Ты вводишь буквы...\nВведи ID')
#СЛИТО В @EDITSOFTED
